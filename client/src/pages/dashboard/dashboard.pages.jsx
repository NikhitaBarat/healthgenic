...
const Dashboard = () => {
  ...
  const getNutritions = async () => {
    try {
       const response = await axios.get(`/api/nutrition/${quantity}/${unit}/${search}`)
    ...
    <div className='dashboard-page'>
      <div className="page-content">
        ...
        <div className="search-bar">
          <input type="text" className="inp-search" onChange={handleSearch} value={search} placeholder='search diet' />
        </div>
        ...
        <div className="qty-section">
          ...
          <div className="unit-sec">
            <h2>Quantity</h2>
            <input type="number" onChange={handleQuantity} value={quantity} name="" id="" />
          </div>
        </div>
          ...
          <div className="nutrients-content">
            <div className="left-ntr">
              <div className='ntr-item'>
                ...
                <p>{data?.totalNutrientsKCal.FAT_KCAL.quantity}{data?.totalNutrientsKCal.FAT_KCAL.unit}</p>
              </div>
              
              <div className='ntr-item'>
                <h3>{data?.totalNutrientsKCal.ENERC_KCAL.label}</h3>
                <p>{data?.totalNutrientsKCal.ENERC_KCAL.quantity}{data?.totalNutrientsKCal.ENERC_KCAL.unit}</p>
              </div>
            </div>
            ...
            <div className="right-ntr">
              <div className='ntr-item'>
                ...
                <p>{data?.totalNutrientsKCal.CHOCDF_KCAL.quantity}{data?.totalNutrientsKCal.CHOCDF_KCAL.unit}</p>
              </div>

              <div className='ntr-item'>
                <h3>{data?.totalNutrientsKCal.PROCNT_KCAL.label}</h3>
                <p>{data?.totalNutrientsKCal.PROCNT_KCAL.quantity}{data?.totalNutrientsKCal.PROCNT_KCAL.unit}</p>
              </div>
            </div>
